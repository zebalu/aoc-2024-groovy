...
def split = { n ->
    def str = Long.toString(n)
    [str.substring(0, str.size().intdiv(2)) as long, str.substring(str.size().intdiv(2)) as long]
}.memoize()
...
size = { stone, steps ->
    if (steps == 0) 1L
        ...
        def (begin, end) = split(stone)
        size(begin, steps - 1) + size(end, steps - 1)
